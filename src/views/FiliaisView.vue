...
const allVisibleSelected = computed(
  () => tableList.value.length > 0 && tableList.value.every((b) => selectedIds.value.has(b.id))
);
...
function toggleSelectAll() {
  if (allVisibleSelected.value) {
    ...
    selectedIds.value = new Set(tableList.value.map((b) => b.id));
  }
}

async function handleBulkDelete() {
  const list = selectedBranches.value;
  const n = list.length;
  if (!n) return;
  const ok = await confirm({
    title: `Excluir ${n} filial(ais)?`,
    message: "As filiais selecionadas serão removidas permanentemente. Essa ação não pode ser desfeita.",
    confirmText: `Excluir ${n}`,
    danger: true
  });
  if (!ok) return;
  list.forEach((b) => deleteBranchRecord(b.id));
  selectedIds.value = new Set();
  resetForm();
  toast(`${n} filial(ais) excluída(s).`);
}
