/* =========================================================
   ...
   ---------------------------------------------------------
   toXLSX()       : arquivo .xlsx com 3 planilhas
   toCSV()        : planilha de lançamentos em .csv
   template()     : modelo .xlsx para importação
   importFile()   : importa planilha verificando duplicados
   ========================================================= */
...
const Export = {
  ...
  toXLSX() {
    if (typeof XLSX === "undefined") {
      ui.toast("Biblioteca de exportação indisponível.");
      return;
    }
    ...
    const indicatorStates = (typeof Employees !== "undefined" && Employees.activeStates)
      ? Employees.activeStates()
      : STATES;
    const indicatorRows = [
    ...
    INDICATORS.forEach((ind) => {
      indicatorStates.forEach((state) => {
        const list = Storage.getEntriesFor(ind.id, state);
    ...
    const sheetEntries = XLSX.utils.aoa_to_sheet(this.buildEntryRows());
    ...
    employees.forEach((e) => {
      const cost = Storage.getLatestForMeta("custo_contratacao", "employeeId", e.id);
      employeeRows.push([
        e.name,
        e.sector,
        e.user,
    ...
    const filename = `gente-gestao-dados_${todayISO()}.xlsx`;
  ...
  toCSV() {
    ...
    const workbook = XLSX.utils.book_new();
    const sheet = XLSX.utils.aoa_to_sheet(this.buildEntryRows());
  ...
  template() {
    if (typeof XLSX === "undefined") {
      ui.toast("Biblioteca indisponível.");
      return;
    }
    const workbook = XLSX.utils.book_new();
  ...
  importFile(file) {
    if (!file) return;
    if (typeof XLSX === "undefined") {
      ui.toast("Biblioteca de importação indisponível.");
      return;
    }
    ...
    reader.onload = (e) => {
      try {
        ...
        const summary = this.importWorkbook(wb);
        ui.renderAll();
        if (typeof Equipe !== "undefined" && Equipe.els && Equipe.els.tableBody) {
          Equipe.renderTable();
        }
        if (typeof Filiais !== "undefined" && Filiais.els && Filiais.els.tableBody) {
          Filiais.renderTable();
        }
        const parts = [];
        parts.push(`${summary.imported} lançamento(s) importado(s)`);
        ...
        ui.toast("Importação concluída — " + parts.join(" · "));
      } catch (err) {
        ui.toast("Erro ao importar a planilha.");
        console.error(err);
      }
    };
  ...
  importWorkbook(wb) {
    ...
    if (entriesSheet) {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        const estado = STATES.includes(estadoRaw)
          ? estadoRaw
          : (typeof ui !== "undefined" && ui.currentState && ui.currentState !== "todos" ? ui.currentState : null);

        const existing = Storage.getEntriesFor(ind.id);
        const duplicate = existing.some(
          (e) => e.date === date && Number(e.value) === value && (e.meta ? e.meta.estado : null) === estado
        );
        if (duplicate) { summary.duplicates++; continue; }

        Storage.addEntry(ind.id, { date, value, state: estado });
        summary.imported++;
      }
    }
    ...
    if (teamSheet) {
      ...
      /* Localiza colunas pelo cabeçalho — compatível com planilhas antigas (sem Entrada) */
      const headerRow = rows[0] || [];
      const findCol = (...names) => {
        const targets = names.map((n) => n.toLowerCase());
        return headerRow.findIndex((h) => targets.includes(String(h ?? "").trim().toLowerCase()));
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        const duplicateEmp = Storage.getEmployees().some(
        ...
        Storage.upsertEmployee(employee);
    ...
    if (filiaisSheet) {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        const existing = Storage.getBranches().find(
